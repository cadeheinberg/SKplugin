...
public class G5_Sumo {
	...
	public static void doRightClick(Player player) {
		if (player.getCooldown(F5_Sumo.getWeapon().getWeaponItem().getType()) > 0) {
			return;
		}
		player.setCooldown(F5_Sumo.getWeapon().getWeaponItem().getType(), F_Stats.getTicksList(F5_Sumo.getKitID())[0]);
	}
